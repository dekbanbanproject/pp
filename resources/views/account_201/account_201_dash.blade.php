...
<div class="tabs-animation">
    ...
    <div id="preloader">
        ...
        <form action="{{ URL('account_201_dash') }}" method="GET">
            @csrf
            <div class="row">
                <div class="col"></div>
                <div class="col-md-5">
                    <h4 class="card-title" style="color: #0ea080">Detail 1102050101.201</h4>
                        ...
                        <div class="table-responsive p-3">
                            <table id="example" class="table table-sm table-striped table-bordered dt-responsive nowrap myTable"
                                ...
                                <tbody>
                                    ...
                                    @foreach ($datashow as $item)
                                        <?php
                                            ...
                                            // STM
                                            $stm_ = DB::select(
                                                'SELECT sum(debit_total) as debit_total,sum(stm_money) as stm_money,count(DISTINCT vn) as Countvisit FROM acc_1102050101_201
                                            ...
                                            // ยกไป
                                            $yokpai_ = DB::select(
                                                'SELECT sum(debit_total) as debit_total,count(DISTINCT vn) as Countvi FROM acc_1102050101_201
                                        ...
                                        ?>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-end" width="10%">  <a href="{{url('account_201_detail/'.$item->months.'/'.$item->year)}}" target="_blank" style="color:rgb(186, 75, 250)"> {{ number_format($sum_Y, 2) }}</a></td>
                                                ...
                                                <td class="text-end" style="color:rgb(4, 161, 135)" width="10%">
                                                    <a href="{{url('account_201_stm/'.$item->months.'/'.$item->year)}}" target="_blank" style="color:rgb(4, 161, 135)"> {{ number_format($sum_stm_money, 2) }}</a>
                                                </td>
                                                <td class="text-end" style="color:rgb(224, 128, 17)" width="10%">
                                                    {{-- <a href="{{url('account_pkucs201_yokpai/'.$item->months.'/'.$item->year)}}" target="_blank" style="color:rgb(224, 128, 17)"> --}}
                                ...
                                <tr style="background-color: #f3fca1">
                                    ...
                                    <td class="text-end" style="background-color: #47A4FA"><label for="" style="color: #0e7fe9">{{ number_format($total1, 2) }}</label></td>
                                    <td class="text-end" style="background-color: #9f4efc" ><label for="" style="color: #970eb3">{{ number_format($total2, 2) }}</label></td>
                                    {{-- <td class="text-end" style="background-color: #c5224b"><label for="" style="color: #FFFFFF">{{ number_format($total3, 2) }}</label></td> --}}
                                    <td class="text-end" style="background-color: #0ea080"><label for="" style="color: #19a582">{{ number_format($total4, 2) }}</label></td>
                                    <td class="text-end" style="background-color: #f89625"><label for="" style="color: #e6761c">{{ number_format($total5, 2) }}</label></td>
